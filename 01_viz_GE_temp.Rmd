...
bg_chart <- function(base_size = 22) {
  ...
  theme_tam(grid = 'X', base_size = base_size, caption_size = base_size - 16) +
  theme(
    plot.caption = element_text(colour = "#666666", margin = margin(0, 22, 24, 0, "pt")),
...
if(generateVidAllYears) {
  ...
  annot_text <- tibble(
    ...
    label = c(
      ...
      "Genève 2006: record de température moyenne pour le mois de juillet, 23.7°C.",
      "Genève 2015: record de température pour le mois de juillet, 24.2 °C et record de température enregistrée: 39.7°C.",
...
annot_text$label_mb <- sapply(annot_text$label, function(lab) paste0(strwrap(lab, 33), sep="", collapse="\n"))
annot_text$label_dk <- sapply(annot_text$label, function(lab) paste0(strwrap(lab, 50), sep="", collapse="\n"))
  ...
  plotSingleYear <- function(syear) {
    p <- bg_chart (base_size = fontSize) + 
      ...
      labs(caption = "Source: MétéoSuisse | Tribune de Genève | @duc_qn ")
  }
    ...
    for( y in if(dev) (names(plots) %>% tail(16)) else names(plots)) {
      ...
      if(y %in% annot_text$year %>% unique()) {
        ...
        yy <- plots[[y]] + 
          ...
          geom_text(data = dt, aes(x = x, y = y, label = label_mb),
                    hjust = 0, vjust = 1,
                    family = font_rc, 
                    size = fontSize / 3.6,
                    fontface = "italic",
    ...
    for( y in if(dev) (names(plots) %>% tail(16)) else names(plots)) {
      ...
      if(y %in% annot_text$year %>% unique()) {
        ...
        yy <- plots[[y]] + 
          ...
          geom_text(data = dt, aes(x = x + 0.3, y = y, label = label_dk),
                    hjust = 0, vjust = 1,
                    family = font_rc, 
                    size = fontSize / 3.3,
                    fontface = "italic",
